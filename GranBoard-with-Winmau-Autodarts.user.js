...
let __lastUndoAt = 0; // timestamp of last UNDO (ms)
  if (window.__GB_AD_STEP3_V318_INIT__) return;
  ...
  function diagRx(v) {
    GB_DIAG.rx++;
    GB_DIAG.lastRx = String(v || "?");
    renderDiag();
  }
  function diagSent(v) {
    GB_DIAG.sent++;
    GB_DIAG.lastSent = String(v || "?");
    renderDiag();
  }
  function diagUp(v) {
    GB_DIAG.up++;
    GB_DIAG.lastUp = String(v || "?");
    renderDiag();
  }
  function diagCounted(v) {
    GB_DIAG.counted++;
    GB_DIAG.lastCounted = String(v || "?");
    renderDiag();
  }
  function diagRetry(v) {
    GB_DIAG.retry++;
    GB_DIAG.lastRetry = String(v || "?");
    renderDiag();
  }

  async function waitForDiagCountIncrease(beforeCount, timeoutMs = 1000) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
      if (GB_DIAG.counted > beforeCount) return true;
      await new Promise(r => setTimeout(r, 40));
    }
    return GB_DIAG.counted > beforeCount;
  }
  function diagFail(v) {
    GB_DIAG.fail++;
    GB_DIAG.lastFail = String(v || "?").slice(0, 80);
    renderDiag();
  }
  ...
  function armDiagCommit(label) {
    __gbAwaitingCommit = { label: String(label || "?"), armedAt: Date.now() };
    clearTimeout(__gbCommitTimer);
    __gbCommitTimer = setTimeout(() => {
      __gbAwaitingCommit = null;
    }, 1800);
  }

  const __gbDiagObserver = new MutationObserver((mutations) => {
    if (!__gbAwaitingCommit) return;
    if (Date.now() - __gbAwaitingCommit.armedAt < 20) return;
    ...
    const hit = __gbAwaitingCommit;
    __gbAwaitingCommit = null;
    clearTimeout(__gbCommitTimer);
    diagCounted(hit.label);
  });

  setTimeout(() => {
    try {
      __gbDiagObserver.observe(document.body, {
        subtree: true,
        childList: true,
        characterData: true
      });
    } catch (e) {
      diagFail("Commit observer: " + (e?.message || e));
    }
  }, 600);
  ...
  function findUndoButton() {
    // Autodarts new UI (2026-09): Undo is icon-only.
    const icon = document.querySelector('svg[data-icon="arrow-u-turn-up-left"]');
  ...
  document.addEventListener("click", (ev) => {
    ...
    const isNewUiUndo = !!btn.querySelector('svg[data-icon="arrow-u-turn-up-left"]');
    const action = isNewUiUndo ? "UNDO" : normalizeActionFromButtonText(btn.textContent || "");
    if (action === "UNDO") {
      handleUndo("button");
    }
  }, true);
...
/********************************************************************
  ...
  const BOARD = (() => {
    ...
    function findBoardRoot() {
      return document.querySelector('[role="img"][aria-label="Dartboard"]');
    }
    ...
    function findBoardSvg() {
      ...
      const svg = root.querySelector('svg[viewBox="0 0 1000 1000"]');
      return svg ? { root, svg, rect: root } : null;
    }

    function svgPointToClient(svg, pt) {
      // The interactive root and the 1000x1000 board share the same displayed square.
      const root = findBoardRoot();
      const r = (root || svg).getBoundingClientRect();
      const sx = r.width / VB.w;
      const sy = r.height / VB.h;
      return { cx: r.left + pt.x * sx, cy: r.top + pt.y * sy };
    }

    function getReactPointerHandler() {
      const root = findBoardRoot();
      if (!root) return null;
      ...
      for (let i = 0; i < 12 && fiber; i++, fiber = fiber.return) {
        ...
        if (
          p &&
          typeof p.onPointerDown === "function" &&
          typeof p.onPointerUp === "function"
        ) {
          return { root, fiber, el: fiber.stateNode, props: p };
        }
      }
      return null;
    }

    function makeReactPointerEvent(type, el, pointerId, clientX, clientY) {
    ...
    function patchPointerCapture(el) {
      ...
      const oldSet = el.setPointerCapture;
      const oldRelease = el.releasePointerCapture;
      const oldHas = el.hasPointerCapture;

      // Synthetic PointerEvents do not own a real browser pointer ID.
      // Autodarts' new UI calls pointer capture, so intercept it while
      // invoking the React handlers directly.
      try { el.setPointerCapture = () => {}; } catch {}
      try { el.releasePointerCapture = () => {}; } catch {}
      try { el.hasPointerCapture = () => true; } catch {}

      return () => {
        try { el.setPointerCapture = oldSet; } catch {}
        try { el.releasePointerCapture = oldRelease; } catch {}
        try { el.hasPointerCapture = oldHas; } catch {}
      };
    }

    async function fireGesture(kind, n) {
      const board = findBoardSvg();
      if (!board) throw new Error("Autodarts Dartboard not found.");

      const pt = pointForKind(kind, n);
      if (!pt) throw new Error("No point for kind=" + kind + " n=" + n);

      const client = svgPointToClient(board.svg, pt);
      const pointerId = ++pointerSeq;

      diagSent(kind + (n || ""));

      let handler = getReactPointerHandler();
      if (!handler) {
        diagFail("React handler missing");
        throw new Error("Autodarts React pointer handler not found.");
      }

      let restore = patchPointerCapture(handler.el);

      try {
        handler.props.onPointerDown(
          makeReactPointerEvent("pointerdown", handler.el, pointerId, client.cx, client.cy)
        );

        await sleep(400);

        restore();
        handler = getReactPointerHandler();
        if (!handler) throw new Error("Autodarts React pointer handler lost after PointerDown.");
        restore = patchPointerCapture(handler.el);

        if (typeof handler.props.onPointerMove === "function") {
          handler.props.onPointerMove(
            makeReactPointerEvent("pointermove", handler.el, pointerId, client.cx, client.cy)
          );
        }
        ...
        restore();
        handler = getReactPointerHandler();
        if (!handler) throw new Error("Autodarts React pointer handler lost before PointerUp.");
        restore = patchPointerCapture(handler.el);

        armDiagCommit(kind + (n || ""));

        handler.props.onPointerUp(
          makeReactPointerEvent("pointerup", handler.el, pointerId, client.cx, client.cy)
        );

        diagUp(kind + (n || ""));
        await sleep(250);
      } catch (e) {
        diagFail(String(e?.message || e));
        throw e;
      } finally {
        restore();
      }
    ...
    async function fire(kind, n) {
      ...
      // First attempt.
      let beforeCount = GB_DIAG.counted;
      await fireGesture(kind, n);

      // Wait only for Autodarts' real game-state confirmation.
      if (await waitForDiagCountIncrease(beforeCount, 300)) return;

      // No commit -> retry exactly the same dart once.
      diagRetry(label);
      ui.logAdv(`Autodarts did not count ${label} -> retry once`);
      await sleep(80);

      beforeCount = GB_DIAG.counted;
      await fireGesture(kind, n);

      if (!(await waitForDiagCountIncrease(beforeCount, 500))) {
        diagFail(`AD did not count ${label} after retry`);
        ui.logAdv(`WARNING: Autodarts still did not count ${label} after retry`);
      }
    }
...
function maybeScheduleAutoNextAfterThirdDart() {
  ...
  // Serialize BLE/UI injections. The new Autodarts board gesture is asynchronous
  // (PointerDown -> React state update -> PointerMove -> PointerUp). Running two
  // throws at the same time can make Autodarts lose one or more darts.
  let __injectQueue = Promise.resolve();

  function queueInjectTarget(target, sourceLabel) {
    __injectQueue = __injectQueue
      .then(() => injectTarget(target, sourceLabel))
      .catch((e) => ui.logAdv("Queued inject error: " + (e?.message || e)));
    return __injectQueue;
  }
  ...
  function onNotify(event) {
    ...
    for (const f of extractFrames()) {
      ...
      (async () => {
        ...
        if (cleaned === "BTN@") {
          await queueInjectTarget({ __specialNext: true }, "BLE");
          return;
        }
        ...
        await queueInjectTarget(target, "BLE");
